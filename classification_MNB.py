...
# 11%
def start_calssification_MNB(wav):
    try:
        mnb = joblib.load('MNB.model')
        print 'load MNBClassifier successfully'
    except IOError:

        print 'MNB classifer file doesnt exist, Train first'
        a, b = load_data_X_Y('train')
        X, y = shuffle_two_list_X_Y(a, b)
        mnb = MultinomialNB()
        pos_X = abs_X_13dim_list(X)
        mnb.fit(pos_X, y)
        joblib.dump(mnb, 'MNB.model')
        print "update the model"

    predict_result = mnb.predict(load_data_user_chose(wav))
    ...
    try:
        if iter_times > 0:
            ...
            for itemX in range(len(testX)):
                predict_result = mnb.predict(testX[itemX])
                vote_vale = vote_the_max_times(predict_result)
...
def abs_X_13dim_list(list_13dim):
    postiv=[]
    for itme in list_13dim:
        postiv.append(map(abs,itme))
    return postiv
# get_the_iter_accucy()
print get_accucy()
